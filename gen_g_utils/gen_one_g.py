...
def geng_single_heterograph(g2m, m2m, m2g):
    import torch
    import dgl
    ...
    # 创建异质图
    hetero_graph = dgl.heterograph(data_dict=data_dict, num_nodes_dict=num_nodes_dict)

    # 加入节点和边的特征
    hetero_graph.nodes['grid'].data['h'] = torch.FloatTensor(g2m_srcnode_feat)
    hetero_graph.nodes['mesh'].data['h'] = torch.FloatTensor(m2m_node_feat)  # mesh节点的特征来源于m2m
    hetero_graph.nodes['grid'].data['h'] = torch.FloatTensor(m2g_dstnode_feat)

    hetero_graph.edges['g2m'].data['e_h'] = torch.FloatTensor(g2m_edge_feat)
    hetero_graph.edges['m2m'].data['e_h'] = torch.FloatTensor(m2m_edge_feat)
    hetero_graph.edges['m2g'].data['e_h'] = torch.FloatTensor(m2g_edge_feat)
...
if __name__ == '__main__':
    ...
    cast_g = geng_single_heterograph((g2m_senders, g2m_receivers, g2m_srcnode_feat, g2m_dstnode_feat, g2m_edge_feat),
    ...
    import dgl
    dgl.save_graphs('l_graphcast.dgl',cast_g)
    # print(g2m_g)
    # print(m2m_g)
    # print(m2g_g)
